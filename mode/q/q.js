...
(function (mod) {
    ...
    CodeMirror.defineMode('q', function (config) {
        var indentUnit = config.indentUnit, curPunc, keywords = buildRE([
                'abs',
                'acos',
                'aj',
                'aj0',
        ...
        function tokenBase(stream, state) {
            ...
            if ('.' == c && /\d/.test(stream.peek()) || /\d/.test(c)) {
                var t = null;
                stream.backUp(1);
                if (stream.match(/^\d{4}\.\d{2}(m|\.\d{2}([D|T](\d{2}(:\d{2}(:\d{2}(\.\d{1,9})?)?)?)?)?)/) || stream.match(/^\d+D(\d{2}(:\d{2}(:\d{2}(\.\d{1,9})?)?)?)/) || stream.match(/^\d{2}:\d{2}(:\d{2}(\.\d{1,9})?)?/) || stream.match(/^\d+[ptuv]{1}/))
                    t = 'temporal';
                else if (stream.match(/^0[NwW]{1}/) || stream.match(/^0x[\d|a-f|A-F]*/) || stream.match(/^[0|1]+[b]{1}/) || stream.match(/^\d+[chijn]{1}/) || stream.match(/-?\d*(\.\d*)?(e[+\-]?\d+)?(e|f)?/))
                    t = 'number';
